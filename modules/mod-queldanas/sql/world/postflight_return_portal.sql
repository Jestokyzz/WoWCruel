-- Postflight jc-2026-08-28-queldanas-return-portal-v1
...
SELECT
    (SELECT COUNT(*)
      ...
      WHERE g.`guid`=900501 AND g.`id`=900501 AND g.`map`=571
        AND ABS(g.`position_x`-5913.81)<0.01
        AND ABS(g.`position_y`-619.233)<0.01
        AND ABS(g.`position_z`-646.425)<0.01
        AND ABS(g.`orientation`-1.5444)<0.0001
        AND g.`spawnMask`=1 AND g.`phaseMask`=1 AND g.`state`=1
        AND t.`type`=22 AND t.`displayId`=7628 AND t.`Data0`=44876
        AND l.`name`='Портал из Даларана на остров Кель''Данас'
    ...
    (SELECT COUNT(*)
       ...
       JOIN `gameobject_template` d ON d.`entry`=900501
    ...
    (SELECT COUNT(*)
       ...
       JOIN `gameobject_template_addon` d ON d.`entry`=900501
      WHERE s.`entry`=187056 AND NOT (
