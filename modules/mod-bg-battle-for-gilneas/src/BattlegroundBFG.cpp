...
namespace
{
constexpr char BFG_TRACKER_PREFIX[] = "JCBG\x09";

enum class BattleForGilneasNodeAnnouncement
{
    Claimed,
    Assaulted,
    Defended,
...
void SendBattleForGilneasNodeAnnouncement(Battleground* battleground, TeamId teamId, uint8 node,
    BattleForGilneasNodeAnnouncement announcement, Player const* player = nullptr)
{
    static char const* const nodeNames[] = { "Маяк", "Водокачка", "Рудник" };
    static char const* const nodeAccusative[] = { "Маяк", "Водокачку", "Рудник" };

    if (node >= GILNEAS_BG_DYNAMIC_NODES_COUNT)
        return;

    bool const alliance = teamId == TEAM_ALLIANCE;
    char const* const teamName = alliance ? "Альянс" : "Орда";
    std::string const playerName = player ? player->GetName() : "";
    std::string message;

    switch (announcement)
    {
        case BattleForGilneasNodeAnnouncement::Claimed:
            message = playerName + " захватывает " + nodeAccusative[node] +
                "! Если никто не вмешается, " + teamName + " установит контроль через 1 минуту.";
            break;
        case BattleForGilneasNodeAnnouncement::Assaulted:
            message = playerName + " атакует " + nodeAccusative[node] + "!";
            break;
        case BattleForGilneasNodeAnnouncement::Defended:
            message = playerName + " защитил точку «" + nodeNames[node] + "»!";
            break;
        case BattleForGilneasNodeAnnouncement::Captured:
            message = std::string(teamName) + (alliance ? " захватил " : " захватила ") +
                nodeAccusative[node] + "!";
            break;
    }
    ...
    ChatHandler::BuildChatPacket(data,
        alliance ? CHAT_MSG_BG_SYSTEM_ALLIANCE : CHAT_MSG_BG_SYSTEM_HORDE,
        message, LANG_UNIVERSAL);
    battleground->SendPacketToAll(&data);
}

void SendBattleForGilneasAddonMessage(Player const* player, std::string const& payload)
{
    std::string message = BFG_TRACKER_PREFIX + payload;
    WorldPacket data;
    data.Initialize(SMSG_MESSAGECHAT, 1 + 4 + 8 + 4 + 8 + 4 + message.size() + 1 + 1);
...
{
    if (_trackerUpdateTimer <= diff)
    {
        BroadcastTrackerState();
        _trackerUpdateTimer = 15000;
    }
    else
        _trackerUpdateTimer -= diff;
        ...
        while (uint32 eventId =_bgEvents.ExecuteEvent())
            switch (eventId)
            {
                ...
                case BG_BFG_EVENT_CAPTURE_MINE:
                {
                    ...
                    SendBattleForGilneasNodeAnnouncement(this, teamId, node,
                        BattleForGilneasNodeAnnouncement::Captured);
...
void BattlegroundBFG::AddPlayer(Player* player)
{
    ...
    SendTrackerState(player);
}

void BattlegroundBFG::SendTrackerState(Player const* player) const
{
    std::string payload = "A=" + std::to_string(m_TeamScores[TEAM_ALLIANCE]) +
        ";H=" + std::to_string(m_TeamScores[TEAM_HORDE]) +
        ";M=" + std::to_string(GILNEAS_BG_MAX_TEAM_SCORE) +
        ";BA=" + std::to_string(_controlledPoints[TEAM_ALLIANCE]) +
        ";BH=" + std::to_string(_controlledPoints[TEAM_HORDE]) +
        ";ACTIVE=" + std::to_string(GetStatus() == STATUS_IN_PROGRESS ? 1 : 0) + ";";
    SendBattleForGilneasAddonMessage(player, payload);
}

void BattlegroundBFG::BroadcastTrackerState() const
{
    for (auto const& entry : GetPlayers())
        if (entry.second)
            SendTrackerState(entry.second);
}
...
void BattlegroundBFG::FillInitialWorldStates(WorldPackets::WorldState::InitWorldStates& packet)
{
    for (uint8 node = 0; node < GILNEAS_BG_DYNAMIC_NODES_COUNT; ++node)
    {
        if (_capturePointInfo[node]._state == GILNEAS_BG_NODE_TYPE_NEUTRAL)
            packet.Worldstates.emplace_back(_capturePointInfo[node]._iconNone, 1);

        for (uint8 i = GILNEAS_BG_NODE_STATUS_ALLY_OCCUPIED; i <= GILNEAS_BG_NODE_STATUS_HORDE_CONTESTED; ++i)
            packet.Worldstates.emplace_back(_capturePointInfo[node]._iconCapture + i-1, _capturePointInfo[node]._state == i ? 1 : 0);
    }
...
void BattlegroundBFG::EventPlayerClickedOnFlag(Player* player, GameObject* gameObject)
{
    ...
    uint32 sound = 0;
    BattleForGilneasNodeAnnouncement announcement = BattleForGilneasNodeAnnouncement::Claimed;
    DeleteBanner(node);
    ...
    else if (_capturePointInfo[node]._state == GILNEAS_BG_NODE_STATUS_ALLY_CONTESTED || _capturePointInfo[node]._state == GILNEAS_BG_NODE_STATUS_HORDE_CONTESTED)
    {
        if (!_capturePointInfo[node]._captured)
        {
            ...
            _capturePointInfo[node]._ownerTeamId = TEAM_NEUTRAL;
            _bgEvents.RescheduleEvent(BG_BFG_EVENT_CAPTURE_LIGHTHOUSE + node, Milliseconds(GILNEAS_BG_FLAG_CAPTURING_TIME));
            announcement = BattleForGilneasNodeAnnouncement::Assaulted;
        }
        else
        {
            ...
            announcement = BattleForGilneasNodeAnnouncement::Defended;
        }
    ...
    SendNodeUpdate(node);
    SendBattleForGilneasNodeAnnouncement(this, player->GetTeamId(), node, announcement, player);
