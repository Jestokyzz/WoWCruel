...
namespace
{
constexpr char TP_TRACKER_PREFIX[] = "JCTP\x09";

void SendTwinPeaksAddonMessage(Player const* player, std::string const& payload)
{
    std::string message = TP_TRACKER_PREFIX + payload;
    WorldPacket data;
    data.Initialize(SMSG_MESSAGECHAT, 1 + 4 + 8 + 4 + 8 + 4 + message.size() + 2);
...
void BattlegroundTP::SendTrackerState(Player const* player) const
{
    std::string payload = "A=" + std::to_string(GetTeamScore(TEAM_ALLIANCE)) +
        ";H=" + std::to_string(GetTeamScore(TEAM_HORDE)) +
        ";M=" + std::to_string(BG_TP_MAX_TEAM_SCORE) +
        ";SA=" + std::to_string(GetFlagState(TEAM_ALLIANCE)) +
        ";SH=" + std::to_string(GetFlagState(TEAM_HORDE)) +
        ";ACTIVE=" + std::to_string(GetStatus() == STATUS_IN_PROGRESS ? 1 : 0) + ";";
    SendTwinPeaksAddonMessage(player, payload);
}

void BattlegroundTP::BroadcastTrackerState() const
{
...
void BattlegroundTP::FillInitialWorldStates(WorldPackets::WorldState::InitWorldStates& packet)
{
  ...
  packet.Worldstates.emplace_back(BG_TP_STATE_TIMER_ACTIVE, GetStatus() == STATUS_IN_PROGRESS ? 1 : 0);
...
uint32 BattlegroundTP::GetAssaultSpellId() const
{
    if ((!GetFlagPickerGUID(TEAM_ALLIANCE) && GetFlagState(TEAM_ALLIANCE) != BG_TP_FLAG_STATE_ON_GROUND) ||
        ...
        _bgEvents.HasTimeUntilEvent(BG_TP_EVENT_BOTH_FLAGS_KEPT10))
    return 0;

    return _bgEvents.HasTimeUntilEvent(BG_TP_EVENT_BOTH_FLAGS_KEPT15) ? BG_TP_SPELL_FOCUSED_ASSAULT : BG_TP_SPELL_BRUTAL_ASSAULT;
}
...
void AddTwinPeaksScripts() {
	...
	Player::bgZoneIdToFillWorldStates[5005] = [](Battleground* bg, WorldPackets::WorldState::InitWorldStates& packet) {
